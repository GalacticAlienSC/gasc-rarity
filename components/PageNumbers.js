...
export const PageNumbers = (props) => {
  ...
  const manyPages = () => {
    return (
      <>
        <a
          className="bg-[#d0e8f5] mr-4 hover:bg-[#e7f0fa] text-[#202520] font-bold py-2 px-4 rounded-md ="
          onClick={() =>
            router.push(
        ...
        {end.map((val) => (
          <a
            className="hidden sm:block bg-[#d0e8f5] mr-4 hover:bg-[#e7f0fa] text-[#202520] font-bold py-2 px-4 rounded-md"
            onClick={() =>
              router.push(
                `?${json2query({ ...router.query, page_id: parseInt(val) })}`
              )
